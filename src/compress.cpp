 ...
 * There does not seem to be a general formula for all paper sizes, hence simple
 * lookup
 */
      ...
      if (algo->reverseLineColumn()) {
        for (unsigned int y = 0; y < localHeight; y++) {
          for (unsigned int x = 0;
               (x < lineWidthInB - hardMarginXInB) && (x < bandWidthInB); x++) {
            band[x * bandHeight + y] =
                planes[i][index + x + hardMarginXInB + y * lineWidthInB];
          }
          for (unsigned int x = lineWidthInB - hardMarginXInB; x < bandWidthInB;
               x++)
  ...
  */
  ...
  while (pageHeight) {
    ...
    if (cmyPlanesHasData) {
      ...
      BandPlane *plane = algo->compress(request, band[page->colorsNr() - 1],
...
static bool _compressWholePage(const Request &request, Page *page) {
  ...
  do {
    ...
    for (unsigned int i = 0; i < page->colorsNr(); i++) {
      ...
      index =
          hardMarginY * (lineWidthInB + 2 * hardMarginXInB) + hardMarginXInB;
      ...
      // Call the compression method
      plane = algo[i].compress(request, buffer, page->width(), planeHeight);
      if (plane) {
        plane->setColorNr(i + 1);
        if (!current)
