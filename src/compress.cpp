 ...
 * Samsung M2026 does not work unless it receives _exactly_ the right
 * bandwidths There does not seem to be a general formula for all paper sizes,
 * hence simple lookup
 */
unsigned long
_get2020BandWidthInB (unsigned char papertype, unsigned long xresolution)
{
  struct Bw
  {
...
static bool
_isEmptyBand (unsigned char *band, unsigned long size)
{
...
static bool
_compressBandedPage (const Request &request, Page *page)
{
  ...
  hardMarginX = ((unsigned long)ceil (page->convertToXResolution (
                     request.printer ()->hardMarginX ()))
                 + 7)
                & ~7;
  hardMarginY
      = ceil (page->convertToYResolution (request.printer ()->hardMarginY ()));
  ...
  if (!(strcasecmp (request.printer ()->model (), "Samsung M2020 Series")))
    {
      bandWidthInB = _get2020BandWidthInB (request.printer ()->paperType (),
                                           page->xResolution ());
      if (!bandWidthInB)
        bandWidthInB = lineWidthInB;
    }
  else
    bandWidthInB = lineWidthInB; // original assumption before M2026 patch
          ...
          if (algo->reverseLineColumn ())
            {
              for (unsigned int y = 0; y < localHeight; y++)
                {
                  for (unsigned int x = 0; (x < lineWidthInB - hardMarginXInB)
                                           && (x < bandWidthInB);
                       x++)
                    {
                      band[x * bandHeight + y]
                          = planes[i][index + x + hardMarginXInB
                                      + y * lineWidthInB];
                    }
                  for (unsigned int x = lineWidthInB - hardMarginXInB;
                       x < bandWidthInB; x++)
...
#ifndef DISABLE_JBIG
static bool
_compressBandedJBIGPage (const Request &request, Page *page)
{
  ...
  hardMarginXInB = ((unsigned long)ceil (page->convertToXResolution (
                        request.printer ()->hardMarginX ()))
                    + 7)
                   / 8;
  hardMarginY
      ...
      if (cmyPlanesHasData)
        {
          for (unsigned int i = 0; i < page->colorsNr (); i++)
            {
              BandPlane *plane
                  = algo->compress (request, band[i], bufferWidth, bandHeight);
              if (plane)
                {
      ...
      else if (!_isEmptyBand (band[page->colorsNr () - 1], bandSize))
        {
          ...
          BandPlane *plane = algo->compress (
              request, band[page->colorsNr () - 1], bufferWidth, bandHeight);
          if (plane)
            {
              plane->setColorNr (4);
              if (!current)
...
static bool
_compressWholePage (const Request &request, Page *page)
{
      ...
      for (unsigned int i = 0; i < page->colorsNr (); i++)
        {
          ...
          index = hardMarginY * (lineWidthInB + 2 * hardMarginXInB)
                  + hardMarginXInB;
          for (unsigned int y = 0; y < pageHeight;
               y++, index += 2 * hardMarginXInB)
            {
...
bool
compressPage (const Request &request, Page *page)
{
  switch (page->compression ())
    {
    case 0x0D:
